...
class TestWebCameraQr(unittest.TestCase):
    ...
    def test_33_does_not_submit_automatically(self):
        js = self._js()
        self.assertNotIn("form.submit(", js); self.assertNotIn("requestSubmit(", js)

    def test_34_detected_token_fills_input(self):
        self.assertIn("tokenInput.value = token", self._js())
    ...
    def test_38_no_fetch(self):
        self.assertNotIn("fetch(", self._js())
    ...
    def test_43_stops_on_submit(self):
        js = self._js(); marker = 'form.addEventListener("submit"'
        self.assertIn(marker, js); self.assertIn("stopCamera();", js[js.index(marker):])

    def test_44_stops_on_pagehide(self):
        self.assertIn('window.addEventListener("pagehide", stopCamera)', self._js())

    def test_45_stops_when_page_hidden(self):
        js = self._js(); self.assertIn('document.addEventListener("visibilitychange"', js)
        self.assertIn("document.hidden", js)

    def test_46_keeps_double_submit_protection(self):
        js = self._js(); self.assertIn('form.dataset.submitted === "true"', js)
        self.assertIn('form.dataset.submitted = "true"', js)
